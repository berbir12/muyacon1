...
export class TaskService {
  ...
  static async getAvailableTasks(userId: string): Promise<Task[]> {
    try {
      console.log('🚀 FIXED TASK SERVICE - Getting available tasks for user:', userId)
      ...
      console.log('🚀 FIXED TASK SERVICE - Filtered tasks count:', data.length, 'out of', allTasks?.length || 0)
      ...
      console.error('🚀 FIXED TASK SERVICE - Error in getAvailableTasks:', error)
  ...
  static async getMyTasks(userId: string): Promise<Task[]> {
    try {
      console.log('🚀 FIXED TASK SERVICE - Getting my tasks for user:', userId)
      ...
      const data = allTasks?.filter(task => task.user_id === userId) || []
      console.log('🚀 FIXED TASK SERVICE - My tasks count:', data.length, 'out of', allTasks?.length || 0)
      ...
      console.error('🚀 FIXED TASK SERVICE - Error in getMyTasks:', error)
  ...
  static async getTaskById(taskId: string): Promise<Task | null> {
    try {
      console.log('🚀 FIXED TASK SERVICE - Getting task by ID:', taskId)
      ...
      console.error('🚀 FIXED TASK SERVICE - Error getting task by ID:', error)
  ...
  static async getFeaturedTasks(): Promise<Task[]> {
    try {
      console.log('🚀 FIXED TASK SERVICE - Getting featured tasks')
      ...
      console.error('🚀 FIXED TASK SERVICE - Error getting featured tasks:', error)
  ...
  static async createTask(taskData: any): Promise<Task | null> {
    try {
      console.log('🚀 FIXED TASK SERVICE - Creating task:', taskData.title)
      ...
      console.log('🚀 FIXED TASK SERVICE - Task created successfully:', data.id)
      return data
    } catch (error) {
      console.error('🚀 FIXED TASK SERVICE - Error creating task:', error)
  ...
  static async applyToTask(taskId: string, taskerId: string, proposedPrice: number, message: string, availabilityDate: string, userId: string): Promise<boolean> {
    try {
      console.log('🚀 FIXED TASK SERVICE - Applying to task:', taskId)
      ...
      console.error('🚀 FIXED TASK SERVICE - Error applying to task:', error)
      return false
    }
  }
  ...
  static async hasUserAppliedToTask(taskId: string, userId: string): Promise<boolean> {
    try {
      console.log('🚀 FIXED TASK SERVICE - Checking if user applied to task:', taskId, userId)
      ...
      console.error('🚀 FIXED TASK SERVICE - Error checking application:', error)
      return false
    }
  }
  ...
  // Get recent tasks (for home page)
  static async getRecentTasks(limit: number = 5): Promise<Task[]> {
    try {
      console.log('🚀 FIXED TASK SERVICE - Getting recent tasks')
      ...
      console.error('🚀 FIXED TASK SERVICE - Error getting recent tasks:', error)
      const appError = handleError(error, 'getRecentTasks')
      throw appError
    }
  }
}
