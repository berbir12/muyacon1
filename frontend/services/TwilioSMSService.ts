...
export class TwilioSMSService {
  ...
  static async sendVerificationCode(phone: string): Promise<{ success: boolean, error?: string }> {
    try {
      ...
      // Send SMS via Twilio using Supabase Edge Function
      const { data, error } = await supabase.functions.invoke('send-sms', {
        body: {
          phoneNumber: formattedPhone,
          message: `Your Muyacon verification code is: ${code}. This code expires in 5 minutes.`
        }
      })
      ...
      if (!data.success) {
        console.error('Twilio function error:', data.error)
